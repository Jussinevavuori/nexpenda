import { unique } from "../generic/unique";
import { compareDate } from "./compareDate";
import { deserializeDate, serializeDate } from "./serializeDate";
...
export function groupByDate<T>(
  ...
  const serializedDates = dates.map(serializeDate);
  ...
  keys.forEach((serial) => {
    const date = deserializeDate(serial);
