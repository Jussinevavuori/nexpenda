...
export function BudgetSummaryField(props: BudgetSummaryFieldProps) {
	...
	return <Tooltip value={<div className="p-2 max-w-xs">
		...
		<div className="flex flex-col gap-2 items-start">
			...
			<p className={c("text-2xl font-semibold", c.variant(props.color)({
				success: "text-success",
				danger: "text-danger",
				neutral: "",
				auto: props.value === 0 ? "" : props.value > 0 ? "text-success" : "text-danger",
				"auto-flipped": props.value === 0 ? "" : props.value > 0 ? "text-danger" : "text-success"
			}))}>
				{(() => {
					switch (props.type) {
						case "default":
						...
						case "money":
							if (typeof props.value !== "number") {
								throw new Error("BudgetSummaryField with type money must be provided a numeric value")
							}
